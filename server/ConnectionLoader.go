...
func (s *Server) deleteS2SRegistrantBlacklist(info protocol.S2SInfo) { //从s2sRegistrant的黑名单中删除一项

	s2sBlacklist := <-s.s2sBlacklist //取出服务器定义的黑名单
	for _, binfo := range s2sBlacklist {
		if binfo.(protocol.S2SInfo).GetRegistryID() == info.GetRegistryID() { //如果在黑名单里
	...
	for i, binfo := range blacklist {
		if binfo.(protocol.S2SInfo).GetRegistryID() == info.GetRegistryID() { //找到匹配的S2SInfo，删除之
...
func (s *Server) deleteCandidate(info protocol.S2SInfo) { //从s2sRegistry和s2cRegistry的候选连接列表中删除一项
	s.deleteS2SCandidate(info)
	s.deleteS2CCandidate(info.S2CInfo)
}

func (s *Server) deleteS2CCandidate(info protocol.S2CInfo) { //从s2cRegistry的候选连接列表中删除一项
	s2cInfo := s.s2cRegistry.Info.(protocol.S2CInfo)              //取出注册中心信息
...
func (s *Server) deleteS2SCandidate(info protocol.S2SInfo) { //从s2sRegistry的候选连接列表中删除一项
	s2sInfo := s.s2sRegistry.Info.(protocol.S2SInfo)              //取出注册中心信息
